...
class SimpleLifecycleNode(Node):
    ...
    def msgCallback(self, msg):
        ...
        if(current_state[1] == 'active'):
            self.get_logger().info(f"Lifecycle node heard: {msg.data}")
    
def main():
    rclpy.init()
    executor = rclpy.executors.SingleThreadedExecutor() #this creates a single thread to execute a portion of a huge logic
    simple_lifecycle_node = SimpleLifecycleNode("simple_lifecycle_node")
    executor.add_node(simple_lifecycle_node) #this will add the class to execute in the thread
    try:
        executor.spin() #this will continuously spin the thread
